export const anecdotesObject = {
    appleVitamin: {
        text: 'Aujourd’hui, les pommes contiennent moins de vitamines C qu’en 1960 !',
        icon: require('~/domain/entities/assets/icon/anecdote-icon/icon-anecdote-apple.svg')
    },

    avocadoFat: {
        text: 'L’avocat contient environ 20 % de matières grasses, dont les 3/4 sont des bons acides gras saturés.',
        icon: require('~/domain/entities/assets/icon/anecdote-icon/icon-anecdote-avocado.svg')
    },

    cigarretteButtPollution: {
        text: 'Un mégot peut polluer jusqu’à 500 litres d’eau. Environ 1000 sont jetés par terre chaque seconde en France.',
        icon: require('~/domain/entities/assets/icon/anecdote-icon/icon-anecdote-water.svg')
    },

    seaCarbonDioxide: {
        text: 'L’océan absorbe près de 30 % des émissions de CO2. Il est de ce point de vue plus performant que les forêts.',
        icon: require('~/domain/entities/assets/icon/anecdote-icon/icon-anecdote-sea.svg')
    }
};
